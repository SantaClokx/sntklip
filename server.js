...
app.post("/extract", async (req, res) => {
  ...
  if (!url || !url.includes("facebook.com")) {
    return res
      .status(400)import express from "express";
import { chromium } from "playwright";
...
app.listen(PORT, () => console.log(`Extraction test service running on port ${PORT}`));
  ...
  try {
    ...
    return res.status(500).json({
      success: false,import express from "express";
import { chromium } from "playwright";
...
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`Extraction test service running on port ${PORT}`));app.postfacebook.comreq.bodyhttps://www.facebook.com/share/r/1Bhn7i7wL7/process.env.PORT30000foundUrls.sizehttps://github.com/nextlevelbuilder/ui-ux-pro-max-skill install this skill
      error: err.message,
      debugLog,
    });
  }
